...
int main(int argc, char *argv[])
{
    ...
    HighFive::File file_final("./disp2_robustness.hdf5", HighFive::File::ReadWrite | HighFive::File::Create | HighFive::File::Truncate);
    ...
    for (const auto &folder : folders)
    {
        ...
        for (size_t seq_idx = 0; seq_idx < SEQUENCES.size(); seq_idx++)
        {
            ...
            for (auto &&cam : CAMS)
            {
                ...
                for (size_t fr_idx = 0; fr_idx < frames.size(); fr_idx++)
                {
                    ...
                    filepath = basepath + "/" + folder + "/test/" + filepath + ".dsp5";
        ...
        std::vector<HighFive::float16_t> second_subsampled_data;
        ...
        second_props.add(HighFive::Chunking{second_subsampled_data.size()});
        second_props.add(HighFive::Deflate(9));

        HighFive::DataSet dataset_second = group.createDataSet<HighFive::float16_t>("disparity",
